...
payload="$(python3 - "${MODEL}" "${TEXT}" "${ref_data}" <<'PY'
import json
import sys
...
kubectl --kubeconfig "${KUBECONFIG_PATH}" -n "${NAMESPACE}" port-forward "svc/${SERVICE}" "${PORT}:80" >/tmp/gpu-ops-tts-port-forward.log 2>&1 &
pf_pid="$!"
...
echo "tts streaming synthesis check passed"
